...
public class AdvancedCarPhysicsWithShader : MonoBehaviour
{
    ...
    [Header("Skid Mark System")]
    public Material skidMarkMaterial;
    public Texture2D skidMarkTexture;
    public Texture2D skidNormalMap;
    public float skidMarkWidth = 0.3f;
    public float skidMarkLengthScale = 1.0f;
    public float skidMarkMaxOpacity = 0.8f;
    public float skidFadeSpeed = 0.05f;
    public int maxSkidSegments = 100;
    ...
    private Queue<SkidMarkWithShader> skidMarkPool = new Queue<SkidMarkWithShader>();
    private List<SkidMarkWithShader> activeSkidMarks = new List<SkidMarkWithShader>();
    private List<SkidMarkWithShader> fadingSkidMarks = new List<SkidMarkWithShader>();
    ...
    {
        // Initialize skid mark pool
        for (int i = 0; i < skidMarkPoolSize; i++)
        {
            GameObject skidObj = new GameObject($"SkidMark_{i}");
            skidObj.transform.SetParent(transform);
            skidObj.SetActive(false);
            SkidMarkWithShader skidMark = skidObj.AddComponent<SkidMarkWithShader>();
            skidMark.Initialize(skidMarkMaterial, skidMarkTexture, skidNormalMap, skidMarkWidth, 
                                skidFadeSpeed, maxSkidSegments, minDistanceBetweenPoints);
    ...
    {
        // Assign smoke and dust to wheels
        for (int i = 0; i < wheels.Count; i++)
        {
            if (i < tireSmokeInstances.Count)
            {
                ParticleSystem smoke = tireSmokeInstances[i];
                smoke.transform.SetParent(wheels[i].suspensionTransform);
                smoke.transform.localPosition = Vector3.zero;
                smoke.Stop();
            }

            if (i < dustInstances.Count)
            {
                ParticleSystem dust = dustInstances[i];
                dust.transform.SetParent(wheels[i].suspensionTransform);
    ...
    private void Update()
    {
        if (enablePerformanceOptimizations && mainCamera != null)
        {
    ...
    private void UpdateEngineAudio()
    {
        ...
        if (engineRPM < idleRPM * 1.2f)
        {
            if (engineAudioSource.clip != engineIdleClip && engineIdleClip != null)
            {
        ...
        {
            if (engineAudioSource.clip != engineRunningClip && engineRunningClip != null)
            {
    ...
    private void UpdateWheelEffects()
    {
        ...
        // Update fading skid marks
        for (int i = fadingSkidMarks.Count - 1; i >= 0; i--)
        {
            SkidMarkWithShader skidMark = fadingSkidMarks[i];
            if (skidMark.IsFinished)
            {
                skidMark.gameObject.SetActive(false);
                skidMarkPool.Enqueue(skidMark);
                fadingSkidMarks.RemoveAt(i);
            }
        }
        ...
        {
            Wheel wheel = wheels[i];
            ParticleSystem smoke = i < tireSmokeInstances.Count ? tireSmokeInstances[i] : null;
            ParticleSystem dust = i < dustInstances.Count ? dustInstances[i] : null;
            
            if (wheel.isGrounded)
            {
                Vector3 contactPoint = wheel.groundHit.point + Vector3.up * 0.01f;
                Vector3 groundNormal = wheel.groundHit.normal;
                ...
                if (dust != null && currentSpeed > minSpeedForDust)
                {
                    ...
                    if (wheel.groundHit.collider.CompareTag("Dirt") || 
                        (wheel.groundHit.collider.sharedMaterial != null && 
                         wheel.groundHit.collider.sharedMaterial.name.Contains("Dirt")))
                    {
                ...
                if (wheel.isSlipping)
                {
                    if (activeSkidMarks.Count <= i || activeSkidMarks[i] == null)
                    {
                        ...
                        {
                            SkidMarkWithShader skidMark = skidMarkPool.Dequeue();
                            skidMark.gameObject.SetActive(true);
                            skidMark.transform.position = contactPoint;
                            skidMark.SetProperties(skidColor, skidMarkMaxOpacity, textureTiling, normalStrength);
                    ...
                    if (activeSkidMarks.Count > i && activeSkidMarks[i] != null)
                    {
                        activeSkidMarks[i].AddPoint(contactPoint, groundNormal, wheel.slipIntensity);
                    }
                }
                else
                {
                    if (activeSkidMarks.Count > i && activeSkidMarks[i] != null)
                    {
                        ...
                        fadingSkidMarks.Add(activeSkidMarks[i]);
            ...
            {
                // Disable effects when wheel leaves ground
...
public class SkidMarkWithShader : MonoBehaviour
{
    ...
    public bool IsFinished => isFinished && fadeFactor <= 0;

    public void Initialize(Material baseMaterial, Texture2D texture, Texture2D normalTexture,
                          float width, float fadeSpeed, int maxSegments, float minDistance)
    {
    ...
    public void SetProperties(Color color, float maxOpacity, float tiling, float normalStrength)
    {
    ...
    {
        // Adjust skid mark quality (simplified for this example)
        minDistance = Mathf.Lerp(0.2f, 0.05f, quality);
    }

    public void AddPoint(Vector3 position, Vector3 normal, float intensity)
    {
        // Don't add points if we're finished
        ...
        else
        {
            ...
            Vector3 lastPosition = vertices[vertices.Count - 2];
            if (Vector3.Distance(lastPosition, position) > minDistance)
            {
                shouldAdd = true;
            }
        }
        ...
        if (vertices.Count >= 2)
        {
            direction = (position - vertices[vertices.Count - 2]).normalized;
        }

        Vector3 right = Vector3.Cross(direction, normal).normalized * width * 0.5f;
        ...
        if (vertices.Count > 2)
        {
            float distance = Vector3.Distance(vertices[vertices.Count - 2], vertices[vertices.Count - 4]);
            uvY = uvs[uvs.Count - 2].y + distance * 0.1f;
        }

        uvs.Add(new Vector2(0, uvY));
        uvs.Add(new Vector2(1, uvY));

        // Add colors with intensity-based alpha
        Color colorWithAlpha = material.GetColor("_Color");
        colorWithAlpha.a = currentOpacity * intensity;
        colors.Add(colorWithAlpha);
        colors.Add(colorWithAlpha);

        // Add triangles if we have at least 4 vertices
        if (vertices.Count >= 4)
        {
        ...
        // Cap the number of segments
        if (vertices.Count > maxSegments * 2)
        {
            // Remove oldest segment
            vertices.RemoveRange(0, 2);
            normals.RemoveRange(0, 2);
            uvs.RemoveRange(0, 2);
    ...
    private IEnumerator FadeOut()
    {
        while (fadeFactor > 0)
        {
            fadeFactor -= fadeSpeed * Time.deltaTime;
            material.SetFloat("_FadeFactor", fadeFactor);

            // Update vertex colors with fading alpha
            for (int i = 0; i < colors.Count; i++)
            {
                Color fadedColor = colors[i];
                fadedColor.a *= fadeFactor;
                colors[i] = fadedColor;
            }

            // Update mesh if we still have vertices
            if (colors.Count > 0)
            {
                mesh.colors = colors.ToArray();
            }
